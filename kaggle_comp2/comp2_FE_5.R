...
dataset[, mtarjetas_consumo := rowSums(.SD, na.rm = T), .SDcols =  c("mtarjeta_visa_consumo","mtarjeta_master_consumo")]
...
dataset[, mcaja_ahorro_ranknorm := round(rank(mcaja_ahorro)/.N,6), by = as.character(foto_mes)]
dataset[,mcaja_ahorro := NULL]

dataset[, mprestamos_personales_ranknorm := round(rank(mprestamos_personales)/.N,6), by = as.character(foto_mes)]
dataset[, mprestamos_personales := NULL]

dataset[, mpasivos_margen_ranknorm := round(rank(mpasivos_margen)/.N,6), by = as.character(foto_mes)]
...
dataset[, mtarjetas_consumo_ranknorm := round(rank(mtarjetas_consumo)/.N,6), by = as.character(foto_mes)]
dataset[,mtarjetas_consumo := NULL]
...
dataset[, cdescubierto_preacordado_ranknorm := round(rank(cdescubierto_preacordado)/.N,6), by = as.character(foto_mes)]
...
lagcols <- c("ctrx_quarter",
             ...
             "mprestamos_personales_ranknorm")
...
fwrite(dataset,
       file = "buckets/b1/datasets/competencia_02_FE5.csv.gz",
       sep = ",")
