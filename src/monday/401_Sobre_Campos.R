...
# Cambio en la distribución de las features en la población objetivo/en los grupos definidos por el target.

# Data drifting vs. conceptual drifting. El segundo implica un cambio en la lógica del negocio, operatoria de los 
# clientes con el banco, etc.

# Haciendo una visualización de las distribuciones entre poblacion muestra y test, no parece haber diferencias
# salvo en algunas variables particulares.

# Una forma de manejar el data drifting para var. continuas relevantes y muy pasibles de ser 
# afectadas por drifting (ej. saldos de cuenta en un contexto inflacionario) es discretizar mediante decilado
# percentilado o cualquier otro cuantil. El mejor de los mundos es normalizar a 0 (indiv min) y 1 (indiv max), 
# para que los casos queden uniformemente distribuidos de menor a mayor (sería cuantilar con cuantiles = individuos).
# De esta forma se mantiene la granularidad del dataset pero se maneja el problema del data drift.